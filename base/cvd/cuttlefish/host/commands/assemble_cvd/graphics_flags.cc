...
Result<void> SetGfxstreamFlags(
  ...
  // Asahi's accelerated path currently boots with gfxstream, but the newer ASG
  // transport hits virtio-gpu blob registration failures at runtime. Prefer the
  // older pipe transport on these Mesa/Apple hosts until ASG is known-good.
  if (IsAsahiMesaHost(availability)) {
    gfxstream_transport = kGfxstreamTransportPipe;
  }
